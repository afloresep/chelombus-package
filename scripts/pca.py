...
def main() -> None:
   ...
   parser.add_argument('--resume_chunk', type=int, default=0, help="Resume from a specific chunk.")
   parser.add_argument('--model', type=str, help='Load a PCA model using joblib to avoid the fitting and directly transform the result')
   ...
   "PCA_MODEL": args.model or config.get("MODEL"),
   }) 
   ...
   ipca = IncrementalPCA(pca_n_components=config.PCA_N_COMPONENTS) 
   for file_path in process_input(config.INPUT_PATH):
      ...
      logging.info(f"iPCA fitting done in {config.INPUT_PATH} took {int((end - start) // 3600)} hours, {int(((end - start) % 3600) // 60)} minutes, and {((end - start) % 60):.2f} seconds")
   ...
   logging.info(f"iPCA calculations for {config.INPUT_PATH} took {int((end - start) // 3600)} hours, {int(((end - start) % 3600) // 60)} minutes, and {((end - start) % 60):.2f} seconds")
